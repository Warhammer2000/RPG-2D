...
public class PlayerStats : MonoBehaviour
{
    ...
    private void SetDamageParameters()
    {
        ...
        if (inventory.equipment[0]) PlayerMelDamage += inventory.equipment[0].damage;
        if (inventory.equipment[1]) PlayerDisDamage += inventory.equipment[1].damage;
    }
    private void SetProtectionParameters()
    {
        PlayerProtection = level + (conProtect * Constitution);
        if (inventory.equipment[2]) PlayerProtection += inventory.equipment[2].protection;
    }
